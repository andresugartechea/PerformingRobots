...
const byte CUSTOM_ADDRESS_BYTE = 0x73;      //33       // change as per the above assignment
const int CUSTOM_CHANNEL_NUMBER = 40; //90 // change as per the above assignment
...
*/
  ...
  const int BODY_SERVO_PIN = 18;

  // Neopixel
  const int NEOPIXELPIN = 20;
  const int NUMPIXELS = 128; //64x2
  ...
  if (radio.available(&pipeNum)) {
    ...
    switch (data.stateNumber) {
      case 0:
        ...
        break;
      case 1:
        Serial.print(F("moving head to 180 and drawing rectangle"));
       //head.write(180);
        ...
        break;
      case 2:
        Serial.println(F("moving head to 30"));
       head.write(30);
